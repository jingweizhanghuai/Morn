...
void mImageHist(MImage*img,int cn,MArray *hist)
{
    ...
    mException((cn>=(img->cn)),EXIT,"invalid input");
    ...
    {
        if(cn >= 0)
            mArrayRedefine(hist,256,S32);
        else if(img->cn <= 2)
            mArrayRedefine(hist,4096,S32);
        else
            mArrayRedefine(hist,((img->cn)<<4),S32);
    }
    ...
    {
        if((img->cn == 3)&&(hist->num == 4096))
            hist_num = 16;
        else if((img->cn == 2)&&(hist->num == 4096))
            hist_num = 64;
        else if(img->cn == 2)
            hist_num = (int)(sqrt((double)(hist->num)));
        else
            hist_num = (int)(pow(((double)(hist->num)),(1.0/((double)(img->cn)))));
...
void mHistNormlize(MArray *src,MArray *dst,int norm_data)
{
    ...
    mArrayRedefine(dst,src->num,S32);
