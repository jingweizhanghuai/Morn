...
void endImageHoughLine(void *info)
{
    ...
    if(handle->point != NULL)
        mListRelease(handle->point);
}
#define HASH_ImageHoughLine 0x3d5cf2dd

void mImageHoughLine(MImage *src,MList *list,int count_thresh,int gap_thresh)
{
    mException(INVALID_IMAGE(src),EXIT,"invalid input source");
    mException((count_thresh<=0),EXIT,"invalid input threshold");
    
    if(gap_thresh<=0) gap_thresh=2;
    ...
    if(hdl->valid==0)
    {
        mTableSet(handle->tab,range*2,362,U16);
        
        if(handle->point == NULL)
            handle->point = mListCreate(DFLT,NULL);
    ...
    for(int j=0;j<height;j++)
        for(int i=0;i<width;i++)
        {
            if(data[j][i] == 255)
            {
                locate.x = i;locate.y = j;
                mListWrite(point,DFLT,&locate,sizeof(MImagePoint));
    ...
    for(int j=0;j<range*2;j++)
    {
        for(int i=1;i<361;i++)
        {
            ...
            line.para = sqrt(1.0+line.k*line.k);
            line.count= 0;
    ...
    printf("aa list->num is %d\n",list->num);
    
    for(int n=0;n<40000;n++)
    {
        // printf("n is %d\n",n);
        MImagePoint *p = point->data[n];
        // if(n==346)
        // printf("aaaaaaaaaaa n is %d:p->x is %f,p->y is %f\n",n,p->x,p->y);
        
        for(int i=0;i<list->num;i++)
        {
            // printf("i is %d,list->num is %d\n",i,list->num);
            struct HoughLineInfo *l = list->data[i];
            float d = ABS(p->x*l->k+l->b-p->y)/l->para;
            if(d<1.5f) 
            {
                int flag = 0;
                if(l->count<=0)
                {
                    l->ps.x = p->x;l->ps.y = p->y;
                    l->pe.x = p->x;l->pe.y = p->y;
                    l->count= 1;
                    flag =1;
                }
                else if(((l->ps.x>p->x)==(p->x>l->pe.x))||((l->ps.y>p->y)==(p->y>l->pe.y)))
                {
                    l->count += 1;
                    flag = 1;
                }
                ...
                {
                    float dx,dy;
                    dx = p->x - l->ps.x;  dx = dx*dx;
                    dy = p->y - l->ps.y;  dy = dy*dy;
                    float d1 = dx+dy;
                    
                    dx = p->x - l->pe.x;  dx = dx*dx;
                    dy = p->y - l->pe.y;  dy = dy*dy;
                    float d2 = dx+dy;
                    
                    d = MIN(d1,d2);
                    
                    // if(i==49)
                        // printf("l->pe.x is %f,p->x is %f,l->para is %f,d is %f,gap_thresh is %d\n",l->pe.x,p->x,l->para,sqrt(d),gap_thresh);
                    
                    if(d<gap_thresh*gap_thresh)
                    {
                        if(d == d1) {l->ps.x = p->x;l->ps.y = p->y;}
                        else        {l->pe.x = p->x;l->pe.y = p->y;}
                        l->count=l->count+1;
                    }
                }
                ...
                if(flag==0)
                {
                    ...
                    {
                        printf("p->x is %f,p->y is %f\n",p->x,p->y);
                        printf("ps is (%f,%f),pe is (%f,%f),count is %d\n",l->ps.x,l->ps.y,l->pe.x,l->pe.y,l->count);

                        mListWrite(list,DFLT,l,sizeof(struct HoughLineInfo));
                        l = list->data[list->num-1];
                        l->count=1;
                        l->ps.x = p->x;l->ps.y = p->y;
                        l->pe.x = p->x;l->pe.y = p->y;
                        // EXIT;
