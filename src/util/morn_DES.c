...
int main()
{
    char filename[256];char name[32];
    sprintf(filename,"./%stmp",tmpnam(name));
...
int main()
{
    ...
    uint64_t data = 0x68652074696d6520;
    ToBigEndian("Now is th",(char *)(&data),8);
    // PrintBit(data,64,8);
    printf("data is %llx\n",data);
    
    uint64_t out;
    out = DesEncrypt(data,key);
    
    // PrintBit(out,64,8);
    printf("out is %llx\n",out);
    
    data = DesDecrypt(out,key);
    printf("data is %llx\n",data);
}
*/
