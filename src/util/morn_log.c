...
void _mLogSet(int level_set,int output,const char *filename,int64_t file_size)
{
    ...
    if(output==DFLT) morn_log_file_valid    = (filename!=NULL);
    else             morn_log_file_valid    = ((output|MORN_LOG_FILE)==MORN_LOG_FILE);
    if(output==DFLT) morn_log_console_valid = !morn_log_file_valid;
    else             morn_log_console_valid = ((output|MORN_LOG_CONSOLE)==MORN_LOG_CONSOLE);
    mException((morn_log_file_valid==0)&&(morn_log_console_valid==0),EXIT,"invalid output set");
