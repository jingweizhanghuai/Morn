...
*/
// build_mingw: g++ -O2 -DNDEBUG test_dictionary2.cpp -o test_dictionary2.exe -lmorn -labsl_hash -labsl_city -labsl_low_level_hash -labsl_raw_hash_set
...
void test1()
{
    ...
    for(int n=0;n<10000;n++)
    {
        for(int i=0;i<100;i++) mDictionaryWrite(data[i].data_s,DFLT,&(data[i].data_i),sizeof(int));
        for(int i=0;i<100;i++) int *data_i = (int *)mDictionaryRead(data[i].data_s);
        for(int i=0;i<100;i++) mDictionaryNodeDelete(data[i].data_s);
    }
